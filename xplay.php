...
# MEDIA DIRECTORY - path/url - absolute - uncomment if needed
...
if ($cache === true && file_exists ( $playlist ) && (date("z")-date("z", filemtime($playlist)) >= 1)){
...
$absolute_path = "http://www.lacymorrow.com/play/music";
	...
	foreach($trackArr as $l){
        ...
        $out .= '      <location>'.$absolute_path.'/'.str_replace(' ','%20', htmlspecialchars($i)).'</location>'.PHP_EOL;
      } else {
  			$out .= '      <location>'.$l['path'].'/'.str_replace(' ','%20', htmlspecialchars($i)).'</location>'.PHP_EOL;
      }
		}
